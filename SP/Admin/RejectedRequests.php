...
{
    define('TITLE', 'RejectedRequests');
    define('PAGE', 'RejectedRequests');
}?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <div class="row">
            ...
            <?php
                if(isset($_REQUEST['View']))
                {
                    ...
                    JOIN technician_tb ON assignwork_tb.technician_id = technician_tb.technician_id JOIN inquiry_tb ON assignwork_tb.inquiry_id = inquiry_tb.inquiry_id WHERE status = 'REJECT'  AND assignwork_tb.inquiry_id = '$id'";
                ...
                if(isset($_REQUEST['assign']))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            $sqlupdate = "UPDATE assignwork_tb SET technician_id = '$rassigntechnician', assign_date = '$rassigndate',  status = 'ASSIGN' WHERE inquiry_id = '$rid'";
                ...
                <form class="ms-2  mt-5  "  novalidate action="" method="post">
                    ...
                    <div class="row ">
                        <div class="form-group col-md-4 mt-2">
                            ...
                            <input type="text" class="form-control" name="serviceCharge" id="serviceCharge" value="<?php if(isset($row['service_charge'])){echo $row['service_charge'];}?>" readonly >
                        </div>
                        <div class="form-group col-md-4 mt-2">
                            <i class="fas fa-user pl-2"></i>
                            <label for="rejectassigntech">Previous Technician</label>
                            <input type="text" class="form-control" name="rejectassigntech" id="rejectassigntech" value="<?php if(isset($row['technician_name'])){echo"ID - "; echo $row["technician_id"]; echo" "; echo $row["technician_name"];}?>" readonly >
                        </div>
                        <div class="form-group col-md-4 mt-2">
                            <i class="fa-regular fa-calendar-days"></i>
                            <label for="inputDate">Previous Assign Date</label>
                            <input type="Date" class="form-control" name="rejectassignDate" id="rejectassignDate" value="<?php if(isset($row['assign_date'])){echo $row['assign_date'];}?>" readonly>
                        </div>
                        <div class="form-group col-md-4 mt-2">
                            <i class="fas fa-user pl-2" ></i>
            ...
            <div class="col-sm-4  mb-5">
                <?php
                    $sql = "SELECT * FROM assignwork_tb 
                    JOIN technician_tb ON assignwork_tb.technician_id = technician_tb.technician_id JOIN inquiry_tb ON assignwork_tb.inquiry_id = inquiry_tb.inquiry_id WHERE status = 'REJECT'";
                    $result = $conn->query($sql);
